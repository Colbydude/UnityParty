...
namespace UnityParty
{
    ...
    public class GameplayManager : MonoBehaviour
    {
        ...
        void MoveToSpaceFinished(BoardSpaceGameObject newSpace)
        {
            // Check if we should decrement the move counter.
            if (!newSpace.Flags.HasFlag(SpaceFlags.DoesNotDecrementMoveCounter))
            {
                m_currentMovementAction.SpacesToMove--;
                RefreshCurrentSpacesText();
            }

            if (newSpace.Flags.HasFlag(SpaceFlags.Stoppable))
            {
